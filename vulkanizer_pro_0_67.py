...
"""
==============================================================================
  ВУЛКАНИЗАТОР ПРО альфа v0.67 (ИНТЕЛЛЕКТУАЛЬНЫЙ ОПТИМИЗАТОР & ВРАППЕР) - GUI EDITION
...
class VulkanizerModel:
    ...
    def install_vulkanizer(self, game_info, dxvk_base_dir, preset="Best", vsync=1, buffering=3, hud="fps", enable_logs=False, spoof_vram=False, api_override="DX11"):
        ...
        api = api_override
        self.log(f"Applying strict API selection: {api}")
...
class VulkanizerApp:
    def __init__(self, root):
        ...
        self.root.title("ВУЛКАНИЗАТОР ПРО v0.67")
    ...
    def create_layout(self):
        ...
        title_label.pack()
        subtitle_label = tk.Label(self.sidebar, text="alpha v0.67 wrapper pro", font=("Segoe UI", 8, "italic"), bg=THEME['bg_sidebar'], fg=THEME['text_gray'])
    ...
    def show_game_control_panel(self, game):
        ...
        default_api = game.get('api', 'DX11')
        if default_api not in ["DX11", "DX10", "DX9"]:
            default_api = "DX11"
            
        self.api_override_var = tk.StringVar(value=default_api)
        
        override_frame = tk.Frame(panel, bg=THEME['bg_sidebar'], pady=5, padx=8)
        override_frame.pack(fill=tk.X, pady=(0, 15))
        
        for val in ["DX11", "DX10", "DX9", "All"]:
            tk.Radiobutton(
    ...
    def add_custom_folder(self):
        ...
        if not scanned:
            messagebox.showinfo(self.tr('title'), "В выбранной папке не найдено подходящих .exe файлов.")
            return

        # Создаем всплывающее окно для выбора конкретных файлов
        sel_win = tk.Toplevel(self.root)
        sel_win.title("Выберите EXE файлы")
        sel_win.geometry("500x400")
        sel_win.configure(bg=THEME['bg_card'])
        ...
        tk.Label(sel_win, text="Отметьте файлы для добавления:", font=("Segoe UI", 11, "bold"), bg=THEME['bg_card'], fg=THEME['accent_cyan']).pack(pady=10)

        canvas = tk.Canvas(sel_win, bg=THEME['bg_main'], highlightthickness=0)
        scrollbar = ttk.Scrollbar(sel_win, orient="vertical", command=canvas.yview)
        ...
        for game in scanned:
            ...
            check_vars.append((var, game))
            
            frame = tk.Frame(scrollable_frame, bg=THEME['bg_main'])
            frame.pack(fill=tk.X, pady=2, anchor="w")
            
            cb = tk.Checkbutton(
                frame, text=f"{game['game_name']} ({game['exe_name']})", variable=var,
                bg=THEME['bg_main'], fg=THEME['text_light'], selectcolor=THEME['bg_sidebar'], 
                activebackground=THEME['bg_main'], activeforeground=THEME['accent_cyan'], font=("Segoe UI", 9)
            )
            cb.pack(side=tk.LEFT)

        def apply_selection():
            added_any = False
            for var, game in check_vars:
                if var.get():
                    game['arch'] = self.model.get_exe_architecture(game['path'])
                    game['api'] = self.model.scan_exe_api(game['path'])
        ...
        btn_apply = tk.Button(
            sel_win, text="Добавить выбранные", font=("Segoe UI", 10, "bold"),
            bg=THEME['accent_cyan'], fg=THEME['text_dark'], relief=tk.FLAT, bd=0, padx=15, pady=8,
    ...
    def add_custom_file(self):
        ...
        if file_path:
            p = Path(file_path)
            parent_name = p.parent.name
            # Умное определение названия игры (отсекаем папки bin, x64 и т.д.)
            game_name = p.parent.parent.name if parent_name.lower() in ["bin", "win32", "win64", "x64", "x86"] else parent_name
            ...
            game_info['arch'] = self.model.get_exe_architecture(game_info['path'])
            game_info['api'] = self.model.scan_exe_api(game_info['path'])
            
            # Проверяем, нет ли уже такой игры в списке
            if not any(g['path'] == game_info['path'] for g in self.model.games):
                self.model.games.append(game_info)
                self.update_game_cards()
                # Автоматически выбираем добавленную игру
                self.select_game(game_info)
